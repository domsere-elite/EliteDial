...
import {
    buildPredictiveAnswerHandler,
    PredictiveAnsweredContext,
    AiOverflowBridgeAnsweredContext,
} from '../services/predictive-answer-handler';
...
const makeDeps = (overrides: any = {}) => {
    ...
    const resolveReservationCalls: Array<{ attemptId: string; outcome: 'completed' | 'retry' }> = [];

    return {
        deps: {
            client: {
                createCall: overrides.createCall ?? (async (args: any) => {
                    createCallArgs.push(args);
                    return { callControlId: 'agent-ccid-1' };
                }),
                hangup: async (args: any) => { hangupArgs.push(args); },
                bridge: overrides.bridge ?? (async (args: any) => { bridgeArgs.push(args); }),
            },
            prisma: {
                user: {
            ...
            resolveReservation: async (attemptId: string, outcome: 'completed' | 'retry') => {
                resolveReservationCalls.push({ attemptId, outcome });
            },
            config: {
...
const aiCtx = (over: Partial<AiOverflowBridgeAnsweredContext> = {}): AiOverflowBridgeAnsweredContext => ({
    callControlId: 'ai-ccid-1',
    bridgeWith: 'consumer-ccid-1',
    campaignId: 'camp-1',
    contactId: 'contact-1',
    attemptId: 'attempt-1',
    ...over,
...
describe('predictive-answer-handler', () => {
    ...
    // --- Orphan-reservation cleanup (known_issue #1) ---

    it('releases reservation as completed when AMD detects voicemail', async () => {
        const t = makeDeps({ reservationResult: { count: 0 } });
        const handler = buildPredictiveAnswerHandler(t.deps as any);
        await handler.onPredictiveAnswered(ctx({ answeringMachineDetected: true }));

        assert.deepEqual(t.resolveReservationCalls, [
            { attemptId: 'attempt-1', outcome: 'completed' },
        ]);
    });

    it('releases reservation as retry when agent bridge createCall throws', async () => {
        const t = makeDeps({
            reservationResult: { count: 1 },
            reservedUser: { id: 'user-1', telnyxSipUsername: 'ext100' },
            createCall: async () => { throw new Error('telnyx 90018 call has already ended'); },
        });
        const handler = buildPredictiveAnswerHandler(t.deps as any);
        await handler.onPredictiveAnswered(ctx());

        assert.deepEqual(t.resolveReservationCalls, [
            { attemptId: 'attempt-1', outcome: 'retry' },
        ]);
    });

    it('releases reservation as retry when no overflow number is configured anywhere', async () => {
        const t = makeDeps({
            reservationResult: { count: 0 },
            campaign: { id: 'camp-1', aiOverflowNumber: null },
            globalOverflow: null,
    ...
    it('releases reservation as retry when AI overflow createCall throws', async () => {
        const t = makeDeps({
            reservationResult: { count: 0 },
            globalOverflow: '+12762128412',
            createCall: async () => { throw new Error('boom'); },
    ...
    it('does not release reservation on successful bridge — call is still live', async () => {
        // Happy paths (agent bridge OK, AI overflow createCall OK) must NOT release the
        // reservation — the call is in flight; call.hangup will close it out later.
        const t = makeDeps({
            reservationResult: { count: 1 },
            reservedUser: { id: 'user-1', telnyxSipUsername: 'ext100' },
        });
        ...
        assert.equal(t.resolveReservationCalls.length, 0);
    });

    it('releases reservation as retry when AI overflow bridge fails', async () => {
        const t = makeDeps({
            bridge: async () => { throw new Error('bridge failed'); },
        });
        const handler = buildPredictiveAnswerHandler(t.deps as any);
        await handler.onAiOverflowBridgeAnswered(aiCtx());
