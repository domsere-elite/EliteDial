...
async function seed() {
    ...
    console.log(`   Demo data: ${SEED_DEMO_DATA ? 'yes' : 'no'} (SEED_DEMO_DATA=${process.env.SEED_DEMO_DATA ?? 'unset'}, NODE_ENV=${process.env.NODE_ENV ?? 'unset'})`);

    // ─── Users ──────────────────────────────────
    const adminPasswordPlain = process.env.ADMIN_PASSWORD || 'admin123';
    if (!process.env.ADMIN_PASSWORD && process.env.NODE_ENV === 'production') {
        console.warn('⚠️  ADMIN_PASSWORD env var not set — seeding admin with insecure default. Set ADMIN_PASSWORD and rerun, or change the password immediately after login.');
    }
    ...
    // ─── Phone Numbers ──────────────────────────
    // Sample test numbers — real DIDs should be provisioned via the admin UI
    if (SEED_DEMO_DATA) {
    ...
    console.log(`   Admin: admin / ${process.env.ADMIN_PASSWORD ? '(password from ADMIN_PASSWORD env)' : 'admin123'}`);
    if (SEED_DEMO_DATA) {
